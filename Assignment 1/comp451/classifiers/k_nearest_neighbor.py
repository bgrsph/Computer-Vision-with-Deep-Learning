...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        # (x-y)^2 = x^2 -2xy + y^2 where x = X and y = X_train
        dists = np.sqrt(np.sum(np.square(X), axis=1)[:, np.newaxis] +
                        np.multiply(np.dot(X, self.X_train.T), -2) +
    ...
    def compute_L1_distances_no_loops(self, X):
        ...
        # BUGRA: I've intentionally commented out this section since my computer's RAM is not sufficient enough to do the
        # operation below. I wanted to avoid any run-time errors.

        # | x - y | where x = X and y = X_train //TOO MUCH MEMORY REQUIRED. COULDN'T RUN.
        #dists = np.abs(np.subtract(X,self.X_train))
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
