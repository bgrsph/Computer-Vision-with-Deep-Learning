...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        fc1 = X.dot(W1) + b1
        out_1 = np.maximum(0, fc1)
        fc2 = out_1.dot(W2) + b2
        out_2 = np.maximum(0, fc2)
        scores = out_2.dot(W3) + b3
        ...
        num_train = X.shape[0]
        scores -= scores.max()
        softmaxes = np.exp(scores) / np.exp(scores).sum(axis=1, keepdims=True)
        losses = (-1) * np.log(softmaxes[np.arange(num_train), y])
        loss = np.sum(losses)/num_train + reg * (np.sum(W1 * W1) + np.sum(W2 * W2) + np.sum(W3 * W3))
        ...
        grads['W3'] = out_2.T.dot(dscores) + (2 * reg * W3)
        grads['b3'] = np.sum(dscores, axis=0)

        dout_2 = dscores.dot(W3.T)
        drelu2 = (out_2 > 0) * dout_2

        grads['W2'] = out_1.T.dot(drelu2) + (2 * reg * W2)
        grads['b2'] = np.sum(drelu2, axis=0)

        dout_1 = drelu2.dot(W2.T)
        drelu1 = (out_1 > 0) * dout_1

        grads['W1'] = X.T.dot(drelu1) + (2 * reg * W1)
    ...
    def predict(self, X):
        ...
        out_1 = np.maximum(0, fc1)
        fc2 = out_1.dot(self.params['W2']) + self.params['b2']
        out_2 = np.maximum(0, fc2)
        scores = out_2.dot(self.params['W3']) + self.params['b3']
