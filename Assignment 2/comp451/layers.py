...
def conv_forward_naive(x, w, b, conv_param):
    ...
    convolution = [(i,k,j,l) for i in range(N) for k in range(F) for j in range(Hh) for l in range(Hw)]
    for (i,k,j,l) in convolution:
        out[i,k,j,l] = np.sum(X_pad[i,:,j * S:j * S + HH, l * S:l * S + WW] * w[k,:]) + b[k]
...
def max_pool_forward_naive(x, pool_param):
    ...
    pooling = [(i,j,k,z) for i in range(N) for j in range(C) for k in range(H_) for z in range(W_)]
    for (i,j,k,z) in pooling:
...
def max_pool_backward_naive(dout, cache):
    ...
    back_pooling = [(i,z,j,k) for i in range(N) for z in range(C) for j in range(H_) for k in range(W_)]
    for (i,z,j,k) in back_pooling:
        d = np.zeros((pool_height,pool_width))
        input_ = x[i,z,j*stride:(j*stride+pool_height),k*stride:(k*stride+pool_width)]
        max_ = np.where(input_==input_.max())
        d[max_[0], max_[1]] = dout[i,z,j,k]
        dx[i,z,j*stride:(j*stride+pool_height),k*stride:(k*stride+pool_width)] += d
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
