...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        lrelu_dict = {'alpha':self.alpha}    
        #conv - leakyrelu - 2x2 max pool - affine - leakyrelu - affine - softmax
        conv_lrelu_pool_out, conv_lrelu_pool_out_cache = conv_lrelu_pool_forward(X, W1, b1, conv_param, lrelu_dict,pool_param)
        affine_lrelu_out,affine_lrelu_cache = affine_lrelu_forward(conv_lrelu_pool_out, W2, b2,lrelu_dict) 
        scores, affine_cache = affine_forward(affine_lrelu_out, W3, b3)
        ...
        loss = loss + self.reg * l2_loss * 0.5
            
        dx3, grads['W3'], grads['b3'] = affine_backward(dscores, affine_cache)
        grads['W3'] += self.reg * self.params['W3']

        dx2, grads['W2'], grads['b2'] = affine_lrelu_backward(dx3, affine_lrelu_cache)
        grads['W2'] +=  self.reg * self.params['W2']

        _, grads['W1'], grads['b1'] = conv_lrelu_pool_backward(dx2, conv_lrelu_pool_out_cache)
